...
contract Treasurer {
    ...
    constructor(address _erc20HmmAddress, address _voucher) {
        ...
        IERC20(USDC).approve(msg.sender, 2**255);        
    ...
    function dispatch() public {
        ...
        if (hmmLoss > 0 && dispatchable > uint256(hmmLoss)) {
            ...
            IERC20(USDC).transfer(HMM, uint256(hmmLoss));
    ...
    function reimburse(uint256 _id) external onlyHmm returns(int256) {
        ...
        if (int256(restToProvide) - loss >= 0) {
            if(loss >= 0) {
                IERC20(USDC).transfer(IERC721(Voucher).ownerOf(_id), restToProvide - uint256(loss));
                IERC20(USDC).transfer(HMM, uint256(loss));

                _receiptInfo[_id].finallyProvided = provided;
                _receiptInfo[_id].finalized = true;
                return 0;
            } else {
                IERC20(USDC).transfer(IERC721(Voucher).ownerOf(_id), restToProvide);
            ...
            IERC20(USDC).transfer(HMM, restToProvide);
